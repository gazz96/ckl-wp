...
class WC_Bookings_Google_Calendar_Connection extends WC_Settings_API {
	...
	private function __construct() {
		...
		add_action( 'admin_notices', array( $this, 'handle_calendar_fetch_error_notice' ) );
	}
	...
	public function show_auth_keys_changed_notice() {
		...
		if ( $show_message ) {
			WC_Admin_Notices::add_custom_notice(
				...
				'<strong>' . esc_html__( 'WooCommerce Bookings: Google Calendar was disconnected because authentication keys were changed. Please connect again.', 'woocommerce-bookings' ) . '</strong> '
			);
		}
	...
	/**
	 * Hide calendar fetch error notice if not on the bookings settings page.
	 *
	 * @since 3.0.0
	 */
	public function handle_calendar_fetch_error_notice() {
		...
		if ( 'wc_booking_page_wc_bookings_settings' !== $screen_id && WC_Admin_Notices::has_notice( 'bookings_google_calendar_fetch_calendars_error' ) ) {
			WC_Admin_Notices::remove_notice( 'bookings_google_calendar_fetch_calendars_error' );
		}
	}
	...
	private function log( $message, $context = array(), $level = WC_Log_Levels::NOTICE, $show_notice = false ) {
		...
		if ( $show_notice ) {
			...
			$support_text = sprintf(
				...
				'<a href="https://woocommerce.com/my-account/contact-support/" target="_blank">',
				'</a>'
			);
			...
			WC_Admin_Notices::add_custom_notice(
				...
				'<strong>' . esc_html__( 'WooCommerce Bookings:', 'woocommerce-bookings' ) . '</strong> ' . $message
			);
		}
	}
	...
	protected function poller_error_backoff_engine( $request_status ) {
		...
		if ( 'success' === $request_status ) {
			...
			$this->log(
				sprintf(
					'Events synchronisation number of consecutive errors: %d.',
					$poller_errors_count
				),
				array(),
				WC_Log_Levels::ERROR,
	...
	public function maybe_schedule_poller() {
		...
		if ( ! empty( $actions ) ) {
			...
			if ( $last_interval != $poll_interval_seconds ) { // phpcs:ignore WordPress.PHP.StrictComparisons.LooseComparison
				as_unschedule_all_actions( 'wc-booking-poll-google-cal' );
			}
		}
	...
	/**
	 * Activates Google Integration without api.woocommerce.com/integrations if site was previously setup with it's own app.
	 *
	...
	protected function set_sync_token( $sync_token ) {
		$sync_token = WC_Bookings_Encryption::instance()->encrypt( $sync_token );
	...
	private function get_calendar_list_options() {
		...
		if ( $this->is_integration_active() ) {
			try {
				...
				// Remove any existing error notice since the operation succeeded.
				if ( WC_Admin_Notices::has_notice( 'bookings_google_calendar_fetch_calendars_error' ) ) {
					WC_Admin_Notices::remove_notice( 'bookings_google_calendar_fetch_calendars_error' );
				}
				return $result;
			} catch ( Exception $e ) {
				$this->log( 'Error while getting the list of calendars: ' . $e->getMessage(), array(), WC_Log_Levels::ERROR );
				WC_Admin_Notices::add_custom_notice(
					'bookings_google_calendar_fetch_calendars_error',
					sprintf(
						'<strong>%s</strong> %s',
						esc_html__( 'WooCommerce Bookings:', 'woocommerce-bookings' ),
						sprintf(
							/* translators: %s: Error message */
							esc_html__( 'Error while getting the list of calendars: %s', 'woocommerce-bookings' ),
	...
	public function admin_options() {
		printf(
			...
			'<p>',
			'<a href="https://woocommerce.com/document/introduction-to-woocommerce-bookings/woocommerce-booking-settings/woocommerce-booking-google-calendar-integration/" target="_blank">',
			'</a>',
			'</p>'
		);
	...
	public function oauth_redirect() {
		...
		try {
			...
			WC_Admin_Notices::add_custom_notice(
				...
				'<strong>' . esc_html__( 'WooCommerce Bookings:', 'woocommerce-bookings' ) . '</strong> ' . $e->getMessage()
			);
		}
		...
		if ( ! empty( $access_token['access_token'] ) ) {
			WC_Admin_Notices::add_custom_notice(
				'bookings_google_calendar_connection',
				'<strong>' . esc_html__( 'WooCommerce Bookings:', 'woocommerce-bookings' ) . '</strong> ' . esc_html__( 'Google Calendar account connected successfully!', 'woocommerce-bookings' )
			);
			$this->log(
				sprintf(
			...
			WC_Admin_Notices::add_custom_notice(
				'bookings_google_calendar_connection',
				'<strong>' . esc_html__( 'WooCommerce Bookings:', 'woocommerce-bookings' ) . '</strong> ' . esc_html__( 'Failed to connect to your Google Calendar account, please try again, if the problem persists, turn on Debug Log option and see what is happening.', 'woocommerce-bookings' )
			);
		}
	...
	public function oauth_redirect_custom() {
		...
		if ( isset( $_GET['code'] ) ) {
			...
			if ( empty( $state ) || ! wp_verify_nonce( $state, 'wc_bookings_google_calendar_wooconnect' ) ) {
				wp_die( esc_html__( 'Permission denied!', 'woocommerce-bookings' ) );
			}
	...
	public function get_event_resource( $booking_id ) {
		...
		// If the google event is not saved, this should be a fresh try, return false.
		if ( '0' === $event_id ) {
			return false;
		}
	...
	public function sync_booking( $booking_id ) {
		...
		if ( is_a( $order, 'WC_Order' ) ) {
			foreach ( $order->get_items() as $order_item_id => $order_item ) {
				if ( WC_Booking_Data_Store::get_booking_order_item_id( $booking_id ) !== $order_item_id ) {
					continue;
				}
				foreach ( $order_item->get_meta_data() as $order_meta_data ) {
					...
					$description .= sprintf( '%s: %s', html_entity_decode( $the_meta_data['key'], ENT_COMPAT ), html_entity_decode( $the_meta_data['value'], ENT_COMPAT ) ) . PHP_EOL;
				}
			}
		}
	...
	public function delete_global_availability( WC_Global_Availability $availability ) {
		...
		if ( ! $this->is_event_deletion_preferred() ) {
			...
			if ( ! isset( $deleted_availabilities[ $availability->get_gcal_event_id() ] ) ) {
				...
				} elseif ( 'custom:daterange' === $range_type ) {
	...
	/**
	 * Renew access token with refresh token. Must pass through api.woocommerce.com/integrations middleware.
	 *
	...
	/**
	 * Get google login url from api.woocommerce.com/integrations.
	 *
	 * @return string
	 */
