import{s as r}from"./chunk-zcBpxl6s.min.js";import{t as l}from"./logger-P3tS6JnT.js";import{t as c}from"./tribute.min-Co_QZSpL.min.js";import{t as d}from"./constant-C7ixvPLw.js";var p=r(c(),1),u=[];async function h(t){let n=t.querySelector('input[type="text"]');if(!n)return;let s=document.createRange().createContextualFragment(`
    ...
    `).querySelector("#windpressetch-sort-css-attr-action");wp.hooks.addAction("windpressetch-autocomplete-items-refresh","windpressetch",()=>{}),wp.hooks.doAction("windpressetch-autocomplete-items-refresh");let o=new p.default({containerClass:"windpressetch-tribute-container",autocompleteMode:!0,menuItemLimit:50,noMatchTemplate:"",values:async function(e,a){a(await wp.hooks.applyFilters("windpressetch-autocomplete-items-query",u,e))},lookup:"value",itemClass:"class-item",menuItemTemplate:function(e){let a="";return e.original.color!==void 0&&(a+=`background-color: ${e.original.color};`),e.original.fontWeight!==void 0&&(a+=`font-weight: ${e.original.fontWeight};`),`
                <span class="class-name" data-tribute-class-name="${e.original.value}">${e.string}</span>
                <span class="class-hint" style="${a}"></span>
            `}});o.setMenuContainer=function(e){this.menuContainer=e},o.events.callbacks,o.attach(n);let i=t.querySelector(":scope > span");i instanceof HTMLElement&&s&&(i.style.display="flex",i.appendChild(s),s.querySelector("div.icon-wrapper").addEventListener("click",async()=>{n.value=await d().contentWindow.windpress.module.classSorter.sort(n.value),n.dispatchEvent(new Event("input",{bubbles:!0}))})),t.dataset.windpressInjected="true"}new MutationObserver(()=>{var n;let t=(n=document.evaluate("//div[contains(@class, 'etch-html-block-properties-wrapper')]//label[contains(@class, 'etch-label')]/span[text()='class']",document,null,XPathResult.FIRST_ORDERED_NODE_TYPE,null).singleNodeValue)==null?void 0:n.parentElement;t&&!t.dataset.windpressInjected&&setTimeout(()=>{t.dataset.windpressInjected||h(t)},100)}).observe(document,{subtree:!0,childList:!0}),l("Intellisense: element-attribute-panel.ts module loaded");
